...
import com.salik.nobrokerapp.Dashboard
...
class UserProfile : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (uuid != null) {
            ...
            database.child("user").child(uuid).child("Gov").get().addOnSuccessListener {
                govid=it.value.toString()
                findViewById<TextView>(R.id.textView5).setText(govid)
                Log.i("firebase", "Got value ${it.value}")
            }.addOnFailureListener{
                Log.e("firebase", "Error getting data", it)
            }

            database.child("user").child(uuid).child("Address").get().addOnSuccessListener {
                address=it.value.toString()
                findViewById<TextView>(R.id.textView7).setText(address)
                Log.i("firebase", "Got value ${it.value}")
            }.addOnFailureListener{
                Log.e("firebase", "Error getting data", it)
            }
